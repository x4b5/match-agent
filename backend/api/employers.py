...
async def list_employers():
    ...
    db_docs = await haal_alle_documenten("vacature")

    # Also get raw directories for items not yet processed
    raw_dirs = formatteer_directory_response(WERKGEVERSVRAGEN_DIR)
...
async def _generate_profile_task(map_pad: str, task_id: str):
    ...
    try:
        ...
        resultaat = await profileer_werkgeversvraag(geschoonde_tekst)
        ...
        try:
            ...
            if vector:
                await bewaar_embedding(uuid_val, "vacature", naam, vector)
